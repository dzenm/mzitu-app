...
public class JsoupUtils {
     ...
     * @return
     */
    public static int position(Elements elements, int last) {
        if (elements.size() >= 2) {
     ...
     * @return
     */
    public static Elements selectElements(String url, String css, String query0) {
        try {
     ...
     * @return
     */
    public static Elements selectElements(String url, String css, String query0, String query1) {
        try {
            Document document = Jsoup.connect(url).get();       // 通过Jsoup链接到一个网址，获取到整个document
